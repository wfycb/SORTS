...
def main():
    ...
    for c in (1, 2):
        cmd = (f'ssh {LOADGEN} "{PIN} python3 ~/tb-load.py --host {ENVOY} --port 8080 '
               ...
               f'--csv /var/tmp/calib_c{c}.csv --label calib-c{c}"')
        procs.append(subprocess.Popen(cmd, shell=True,
                                      stdout=open(f"{OUT}/load_c{c}.log", "w"),
                                      stderr=subprocess.STDOUT))
    log(f"부하 기동: 2 코호트 x 700 rps, 워밍업 {WARMUP}s + 본측정 {DURATION}s")
    ...
    with open(f"{OUT}/calib_stress.json", "w") as f:
        json.dump(results, f, ensure_ascii=False, indent=1)
    log(f"-> {OUT}/calib_stress.json")
