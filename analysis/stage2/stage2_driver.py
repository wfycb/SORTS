#!/usr/bin/env python3
"""STAGE2 본 배치 드라이버 (v3 primitive 판) — nohup 무인 완주.
...
MAN = os.environ.get("S2_MANIFEST", os.path.join(EXP, "manifest_stage2.json"))
OUT = os.environ.get("S2_OUTDIR", os.path.join(EXP, "runs", "stage2-20260812"))
...
def anchor_check(rid, rd, spec):
    ...
    if info["react_from_effect_s"] is not None:
