#!/usr/bin/env python3
"""STAGE4 본 배치 분석 — 관측 감쇠 (PREREG_S4). s3_analyze 확장:
  + 감지 실패 여부, 반응(발효 기준), f_c 커버리지, 라우팅 분화도(칸 수·이질도)
...
def analyze(root, rd):
    ...
    # --- STAGE4 추가 지표 ---
    # 감지: 교란 창에서 c1:search 의 이탈 결정(S3 제외 = feasible 에 S3 없음)
    det_ts = None
    n_dec_ticks = 0
    cover_obs = cover_tot = 0
    cells_active = set()
    het_num = het_den = 0
    coh_dst = defaultdict(dict)          # tick -> {cohort: feasible_set} (search)
    for r in csv.DictReader(open(os.path.join(rd, "decisions.csv"))):
        ts = float(r["ts"])
        if r["class"] == "search" and t_on <= ts < t_off:
            coh_dst[round(ts, 2)][r["cohort"]] = r["feasible_set"]
            if (det_ts is None and r["cohort"] == "c1" and r["changed"] == "1"
                    and "S3" not in (r["feasible_set"] or "")):
                det_ts = ts
    for tick, m2 in coh_dst.items():
        vs = list(m2.values())
        n_dec_ticks += 1
        pairs = [(a, b) for i, a in enumerate(vs) for b in vs[i + 1:]]
        het_num += sum(1 for a, b in pairs if a != b)
        het_den += len(pairs)
    for r in rows:                        # obs_state (위에서 로드됨)
        ts = float(r["ts"])
        if t_on <= ts < t_off and int(r["n"]) > 0:
            cover_tot += 1
            cells_active.add((r["site"], r["class"]))
            if r["src"] == "obs":
                cover_obs += 1
    A1v = A1
    react = None if (det_ts is None or A1v is None) else round(det_ts - A1v, 3)

    return {
        "detect": {"detected": det_ts is not None,
                   "react_from_effect_s": react},
        "coverage": {"obs_tick_pct": round(100 * cover_obs / cover_tot, 1)
                     if cover_tot else None,
                     "active_cells": len(cells_active)},
        "divergence": {"het_pair_pct": round(100 * het_num / het_den, 1)
                       if het_den else None},
...
def main():
    ...
    json.dump(out, open(os.path.join(root, "s4_results.json"), "w"),
